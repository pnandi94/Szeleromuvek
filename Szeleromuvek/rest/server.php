...
try {
	...
	switch($_SERVER['REQUEST_METHOD']) {
		case "GET":
			...
			break;
		case "POST":
				$sql = "insert into torony values (0, :db, :teljesitmeny, :kezdev, :helyszinid)";
				$sth = $dbh->prepare($sql);
				$count = $sth->execute(Array(":db"=>$_POST["db"], ":teljesitmeny"=>$_POST["teljesitmeny"], ":kezdev"=>$_POST["kezdev"], ":helyszinid"=>$_POST["helyszinid"]));
		...
		case "PUT":
				...
				if($data['helyszinid'] != "") {$modositando .= ", helyszinid = :helyszinid"; $params[":helyszinid"] = $data["helyszinid"];}
